...
const RETAIL = /в зёрнах|в зернах|котлеты для бургеров|для бургеров|полуфабрикат|дрип[- ]?пакет|в капсул|капсул[аы]|чалд|молотый кофе|кофе молотый|для запекания|для жарки|для гриля|заготовк|набор для|мясо для|(^|[^а-яёa-z0-9])кг([^а-яёa-z0-9]|$)|\d+[.,]?\d*\s*кг|замороженн|охлаждённ|охлажденн|весов(ой|ая|ое)|фасован/i;
